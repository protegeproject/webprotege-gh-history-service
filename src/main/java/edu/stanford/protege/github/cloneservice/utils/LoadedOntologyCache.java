...
public class LoadedOntologyCache {
    ...
    public Optional<OWLOntology> get(OWLOntologyDocumentSource documentSource) {
        ...
        if(ont.isPresent()) {
            cacheHits++;
            logger.debug("Cache hit for ontology load request. [cacheHits={}, documentSource={}]", cacheHits, documentSource.getDocumentIRI());
        }
        else {
            logger.debug("Cache miss for ontology load request. [documentSource={}]", documentSource.getDocumentIRI());
        }
        return ont;
    }
